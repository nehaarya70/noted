...
<body>
	...
	<?php
		...
		if($type=='all')
		{
			$query = "SELECT * FROM uploads  WHERE file_uploaded_to= '$course'  and status='$status' ORDER BY file_uploaded_on DESC";
		}
		
		else{
		
			$query = "SELECT * FROM uploads where file_uploaded_to='".$course."'and category='$type' and status='$status' ORDER BY file_uploaded_on DESC ";
		}
